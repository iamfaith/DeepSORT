...
void test_deepsort(cv::Mat &frame, const std::vector<detect_result> &results, tracker &mytracker)
{
    ...
    cv::imshow("DeepSORT", frame);
}

void test_bytetrack(cv::Mat &frame, const std::vector<detect_result> &results, BYTETracker &tracker)
{
    std::vector<detect_result> objects;
